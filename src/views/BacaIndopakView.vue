...
export default {
  ...
  methods: {
    getSurah()
    {
      axios.get("https://api.quran.com/api/v4/quran/verses/indopak?chapter_number=" + this.cari)
        .then(response =>
        {
...
<template>
  <div class="text-sm-center mt-5">
    ...
    <div v-if="cari" v-for="(ayat,index) in surah" :key="index" class="card">
      <div class="card-body">
        <h5 class="card-title text-end  mt-3">{{ ayat?.text_indopak}}{{ayat?.verse_key}}</h5>
